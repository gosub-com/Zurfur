    ...
    static class CompileCode
    {
            ...
            Symbol currentFunction)
        {
            ...
            Rval GenDot(SyntaxExpr ex)
            {
                ...
                return new Rval(identifier) { InType = leftType};
            }
            ...
            Rval GenNewVarsOperator(SyntaxExpr ex)
            {
                ...
                var symbols = FindRvalSymbols(variables);
                Debug.Assert(symbols.Count == 1 && symbols[0].IsLocal && symbols[0].Type == null);
                symbols[0].Type = valueType;
                variables.Token.AddInfo(symbols[0]);
                return value;
            }
            ...
            Rval GenAssign(SyntaxExpr ex)
            {
                ...
                if (left.IsSetter)
                {
                    ...
                    var args = assignedSymbol.FunParamTuple.TypeArgs;
                    if (args.Length != 2)
                    {
            ...
            Rval GenOperator(SyntaxExpr ex)
            {
                ...
                var rval = FindCompatibleFunction(new Rval(ex.Token, operatorName),
            ...
            Symbol FindAndCall(Token token, Symbol inType, string name)
            {
                ...
                return FindCompatibleFunction(new Rval(token, name) { InType = inType },
                     new List<Rval>(), $"'{name}' in the type '{inType}'", false)?.Type;
            }
                ...
                bool addSymbolInfo = true)
            {
                ...
                return new Rval(call.Token) { 
                    Type = ReplaceGenericTypeParams(funType, funTypeArgs),
            ...
            Rval FindCompatibleConstructor(Rval call, Symbol callType, List<Rval> args)
            {
                ...
                if (callType.IsTypeParam)
                {
                    ...
                    return new Rval(call.Token) { Type = table.GetGenericParam(callType.Order) };
                }
                ...
                if (call.TypeArgs.Length == callType.GenericParamTotal())
                {
                    ...
                    if (call.TypeArgs.Length != 0)
                        callType = table.CreateSpecializedType(callType, call.TypeArgs);
                }
                else
                {
                    Reject(call.Token,
                ...
                if (args.Count == 0)
                {
                    ...
                    return new Rval(call.Token) { Type = callType };
                }
            ...
            CallCompatible IsCallCompatible(Symbol func, Rval call, List<Rval> args)
            {
                if (!func.IsFun)
                ...
                // Type args supplied by the function (e.g. from constraint)
                // and also supplied by the program.
                if (func.TypeArgs.Length != 0 && typeArgs.Length != 0)
                    return CallCompatible.TypeArgsSuppliedByConstraint;

                // Use type args from function
            ...
            bool MatchWithGenerics(Symbol func, Symbol[] typeArgs, Symbol arg, Symbol param)
            {
                if (param.HasGenericArg)
                {
                    if (func.GenericParamTotal() != typeArgs.Length)
                        return false;
                    param = ReplaceGenericTypeParams(param, typeArgs);
                }
                ...
                bool allowStaticType = false)
            {
                ...
                var symbols = FindRvalSymbols(rval);
                if (symbols == null || symbols.Count == 0)
                    return null;

                if (RejectAmbiguousPrimary(rval.Token, symbols))
                    return null;

                // Filter out functions, except getters and setters
                var oldCalls = symbols.ToArray();
                symbols.RemoveAll(callFunc => callFunc.IsFun && !(callFunc.IsGetter || callFunc.IsSetter));
            ...
            List<Symbol> FindRvalSymbols(Rval rval)
            {
                ...
                if (rval.InType == null)
                {
                    symbols = FindGlobal(rval.Token, rval.Name);
                    if (symbols == null)
                        return null;
                }
                else
                {
                    symbols = FindInType(rval.Name, rval.InType);
                    if (symbols.Count == 0)
            ...
            List<Symbol> FindInType(string name, Symbol inType)
            {
                ...
                if (fileUses.UseSymbols.TryGetValue(name, out var useSymbols))
                {
                    foreach (var sym2 in useSymbols)
                        ...
                        {
                            var funParams = sym2.FunParamTuple.TypeArgs;
