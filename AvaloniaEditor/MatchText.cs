...
{
    TokenLoc _startSearchLoc;
    TokenLoc _previousSearchLoc = new TokenLoc(-1, -1);
    ...
    {
        _previousSearchLoc = new TokenLoc(-1, -1);
    }
    ...
    /// </summary>
    public (bool, bool) FindNextAndSelect(Editor editor, string search, bool skipSelectedText)
    {
        ...
        // Start new search?
        if (_previousSearchLoc != start)
        {
            _previousSearchLoc = new TokenLoc(-1, -1);
            _startSearchLoc = start;
        }
        ...
        while (true)
        {
            ...
            if (!firstChar && text.Location == _startSearchLoc)
                pastEnd = true;
            ...
            if (Match(text, find, ref matchEnd))
            {
                ...
                _previousSearchLoc = text.Location;
                return (true, pastEnd);
            }
            text.Inc();
            firstChar = false;
